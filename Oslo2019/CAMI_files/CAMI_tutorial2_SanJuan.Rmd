...
```{r, message=FALSE}
## PICK THE TRAIT HERE
TraitVec <- traitData$Seed.Mass..mg.
...
seedMassTree <- drop.tip(seedMassTree, "Quercus_garryana")
SeedMass <- SeedMass[names(SeedMass) != "Quercus_garryana"]
seedMassTree <- drop.tip(seedMassTree, "Corylus_cornuta")
SeedMass <- SeedMass[names(SeedMass) != "Corylus_cornuta"]

which(SeedMass>350)

hist(SeedMass)
BM.mod <- fitContinuous(phy = seedMassTree, dat = SeedMass, model = "BM", control=list(niter=100))
OU.mod <- fitContinuous(phy = seedMassTree, dat = SeedMass, model = "OU", bounds=list(alpha=c(0.001, 0.01)), control=list(niter=100))

hist(TraitVec)
#RegTraitTree, TraitVec
BM.mod <- fitContinuous(phy = RegTraitTree, dat = log10(TraitVec), model = "BM", control=list(niter=100))
OU.mod <- fitContinuous(phy = RegTraitTree, dat = log10(TraitVec), model = "OU", bounds=list(alpha=c(0.001, 0.02)), control=list(niter=100))
...
```{r}
modelIndex <- c()
modelIndex <- as.character(AssemblyData$params[1:15000,2])
summaryStats <- AssemblyData$summary.stats
ref.table <- c()
ref.table <- data.frame(summaryStats[1:15000,-c(17,18,21,22,29)], modelIndex)
...
trues <- AssemblyData$params[,2]=="filtering"
...
plot(SJModPredictions[,3]~ SJParamPredictions)

lm(SJModPredictions[,2] ~ SJParamPredictions)
